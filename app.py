...
# Timing of Intubation Section
    ...
    with col3:
        ...
        if st.button("Submit"):
            ...
            # Debugging - Check if all required session state variables are present and valid
            formatted_date = st.session_state.get('formatted_date', None)
            formatted_time = st.session_state.get('formatted_time', None)
            front_page_completed = st.session_state.get('front_page_completed', None)
            
            st.write(f"Session State - formatted_date: {formatted_date}")
            st.write(f"Session State - formatted_time: {formatted_time}")
            st.write(f"Session State - front_page_completed: {front_page_completed}")

            # Now check if these values exist and are valid
            if formatted_date and formatted_time and front_page_completed:
                st.write("All required fields are filled in. Proceeding with document creation...")

                # Path to your template file
                template_path = 'airway_bundlex.docx'
